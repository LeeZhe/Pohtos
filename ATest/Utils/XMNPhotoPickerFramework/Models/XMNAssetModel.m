...
+ ( XMNAssetModel * _Nonnull )modelWithAsset:(_Nonnull id)asset type:(XMNAssetType)type timeLength:(NSString * _Nullable )timeLength {
    ...
    NSArray *minuteAndSecond = [timeLength componentsSeparatedByString:@":"];
    model.timeDuration = [minuteAndSecond.firstObject doubleValue] * 60 + [minuteAndSecond.lastObject doubleValue];
    return model;
}
