...
+(NSString *)getTempPath{
     ...
     */
//    return [CustomTools createVideoFolderIfNotExist];
    return nil;
}
...
/**
 *  @author Kiddie, 2016-05-26 15:05:07
 *  @brief 压缩文件
 */
                       ...
                       completeHandler:(void (^)(AVAssetExportSession* exportSession, NSURL* compressedOutputURL)) handler
{
    ...
    if (!outputURL) {
        NSDateFormatter *formater = [[NSDateFormatter alloc] init];//用时间给文件全名，以免重复
        [formater setDateFormat:@"yyyyMMddHHmmss"];
        NSString* fileName = [NSString stringWithFormat:@"compossTemp%@.mp4", [formater stringFromDate:[NSDate date]]];
...
//    NSLog(@"压缩后大小为:%lld", vedioSize);
    ...
    __block int count = 6000;
    NSTimeInterval period = 0.01;
...
//                [//SVProgressHUD showProgress:exportSession.progress status:@"视频处理中..." maskType://SVProgressHUDMaskTypeGradient];
            });
            count--;
        }
    });
    ...
    [exportSession exportAsynchronouslyWithCompletionHandler:^(void) {
        switch (exportSession.status)
        {
            ...
            case AVAssetExportSessionStatusCompleted: {
                dispatch_async(dispatch_get_main_queue(), ^{
//                    [//SVProgressHUD showSuccessWithStatus:@"处理成功" maskType://SVProgressHUDMaskTypeGradient];
                });
                break;
            }
...
/*!
 @method
 @abstract 提取缩略图
 @discussion 通过视频的URL，获得视频缩略图
 @param url 视频URL
 @result 首帧缩略图
 */
+ (UIImage *)getCoverImage:(NSString *)videoURL
{
