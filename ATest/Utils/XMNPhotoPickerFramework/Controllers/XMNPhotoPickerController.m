...
- (instancetype)initWithMaxCount:(NSUInteger)maxCount
    ...
    if (self = [super initWithRootViewController:albumListC]) {
        ...
        _audoPushToVideoCollection = false;
...
- (void)handleAuthorized {
    ...
    if ([[XMNPhotoManager sharedManager] hasAuthorized]) {
        ...
        if (self.audoPushToVideoCollection) {
            // 选手推到视频界面
            [self autoPushVideoCollectionViewController];
            return;
        }
        //已授权
 ...
 *  自动前往视频列表列表页面
 */
- (void)autoPushVideoCollectionViewController{
    ...
    [[XMNPhotoManager sharedManager] getAlbumsPickingVideoEnable:self.pickingVideoEnable completionBlock:^(NSArray<XMNAlbumModel *> *albums) {
        ...
        for (XMNAlbumModel *album in albums) {
            if ([album.name isEqualToString:@"视频"]) {
                photoCollectionC.album = album;
                break;
            }
        }
...
//            [SVProgressHUD showInfoWithStatus:@"相册不存在这张清晰的照片，请到相册列表查询并从iCloud下载"];
            return;
        }
        if (obj.previewImage.size.height / obj.previewImage.size.width > 2) {
            UIImage *scaleImg = [self scaleImage:obj.originImage toScale:[UIScreen mainScreen].bounds.size.width * 1.5 / obj.originImage.size.width];
            [images addObject:scaleImg];
        }else{
